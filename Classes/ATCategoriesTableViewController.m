...
- (void)viewDidLoad {
    ...
    categoryCache = [NSMutableArray arrayWithCapacity:0];
...
- (void)_rebuildCache:(id)sender
{
    ...
    if (count)
    {
        NSDictionary* dict = [NSDictionary dictionaryWithObjectsAndKeys:    kCategorySpecial_UpdatedPackages,                            @"title",
                              ...
                              [NSNumber numberWithInt:(int)count],                                @"count",
                              nil];
    ...
    for (i = 0; i < numSections; i++)
    {
        ...
        NSDictionary* dict = [NSDictionary dictionaryWithObjectsAndKeys:    title,                                                @"title",
                              ...
                              [NSNumber numberWithInt:(int)[[ATPackageManager sharedPackageManager].packages countOfPackagesInCategory:title]], @"count",
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!cell)
        cell = [[ATCategoriesCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"cell" categoriesType:ATCategoriesTYPE_OtherCategories];
    ...
    [cell setCategoriesType:(int)type];
    
    NSString * title = [entry objectForKey:@"title"];
    if (title)
        [cell.textLabel setText:NSLocalizedString(title, @"section title")];
