...
- (void)setPackage:(ATPackage *)aPackage {
	if(aPackage != nil) {
		...
		if (packageFileName)
			self.unpacker = [[ATUnpacker alloc] initWithPath:packageFileName packageID:package.identifier];
	}
}
...
- (BOOL)runScript:(NSArray *)theScript withError:(NSError**)outError {
	...
	for(NSArray * command in theScript) {
		...
		count++;
		percent = (count / [[NSNumber numberWithUnsignedInt:(unsigned int)[theScript count]] doubleValue]) * 100;
		if ([self.delegate respondsToSelector:@selector(scriptDidChangeProgress:progress:)])
...
- (BOOL)script_InstalledPackage:(NSArray *)arguments {
    ...
    SEL scriptIsPackageInstakked = NSSelectorFromString(@"scriptIsPackageInstalled");
	return [[delegate performSelector:scriptIsPackageInstakked withObject:[arguments objectAtIndex:0]] boolValue];
}
...
- (BOOL)script_Exec:(NSArray *)arguments {
	...
	// generate argv
	unsigned arrayCount = (unsigned)[commandArguments count];
...
- (BOOL)script_RunScript:(NSArray *)arguments error:(NSString**)errorDesc
{
	...
	if (!self.lua)
	{
		self.lua = [[ATLuaScript alloc] init];
