...
- (ATSource *)defaultSource {
	ATSource * defaultSource = [[ATSource alloc] init];
...
{
	int updatedCount = (int)[[ATPackageManager sharedPackageManager].packages countOfUpdatedPackages];
	...
	{
        SEL removeApplicationBadge = NSSelectorFromString(@"removeApplicationBadge");
		if ([[UIApplication sharedApplication] respondsToSelector: removeApplicationBadge])
			[[UIApplication sharedApplication] performSelector: removeApplicationBadge withObject:nil];
	}
#endif // INSTALLER_APP
}
...
#warning Removed trusted sources refresh due to URL error
	return [[ATPipelineManager sharedManager] queueTask:[[ATTrustedSourcesRefresh alloc] init] forPipeline:ATPipelineSourceRefresh];
}
...
- (void)scriptIssueError:(ATScript*)script error:(NSString*)error
{
    ...
    UIAlertController * alert = [UIAlertController alertControllerWithTitle:NSLocalizedString(@"Error", @"") message:error preferredStyle:UIAlertControllerStyleAlert];
    
    UIAlertAction * actionCancel = [UIAlertAction actionWithTitle:NSLocalizedString(@"Cancel", @"") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        //Add Action Here
...
- (void)scriptIssueConfirmation:(ATScript*)script arguments:(NSArray*)args
{
    ...
    /*
        Work needs to be done here, to upgrade to newer os
        UIAlertController* aview = [[UIAlertController alloc] init];
    */
...
{
	return [NSNumber numberWithUnsignedInt:(unsigned int)self.scriptConfirmationButton];
}
