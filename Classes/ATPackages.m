...
- (void)rebuild
{
	...
	if (self.customQuery)
	{
		query = [NSString stringWithFormat:@"CREATE TEMP TABLE %@ AS %@ ORDER BY %@ %@ %@", [self sortPackagesTableName],
				 ...
				 self.sortCriteria,	// ORDER BY %@
				 (self.sortAscending) ? @"ASC" : @"DESC",	// ... order by %. %@
                 (self.resultsLimit) ? [NSString stringWithFormat:@"LIMIT %lu", (unsigned long)self.resultsLimit] : @""	// %@ (limit clause)
				 ];		
	}
	else
...
{
    ATResultSet * secID = [[ATDatabase sharedDatabase] executeQuery:[NSString stringWithFormat:@"SELECT _section FROM %@ ORDER BY RowID ASC LIMIT %lu,1", [self packageSectionNamesTableName], (unsigned long)section]];
...
{
    ATResultSet * secID = [[ATDatabase sharedDatabase] executeQuery:[NSString stringWithFormat:@"SELECT TRIM(_section) AS _section FROM %@ ORDER BY RowID ASC LIMIT %lu,1", [self packageSectionNamesTableName], (unsigned long)section]];
	NSString * sectionName = nil;
...
{
    ATResultSet* res = [[ATDatabase sharedDatabase] executeQuery:[NSString stringWithFormat:@"SELECT id FROM %@ ORDER BY RowID ASC LIMIT %lu,1", [self sortPackagesTableName], (unsigned long)index]];
	if (res && [res next])
	{
...
- (ATPackage *)packageAtIndex:(NSUInteger)index ofSection:(NSUInteger)section{
	...
	if (sectionName)
	{
        ATResultSet* packageId = [[ATDatabase sharedDatabase] executeQuery:[NSString stringWithFormat:@"SELECT id FROM %@ WHERE _section = ? ORDER BY RowID ASC LIMIT %lu,1", [self sortPackagesTableName], (unsigned long)(int)index], sectionName];
...
- (ATPackage*)packageWithIdentifier:(NSString*)identifier forSource:(ATSource*)source
{
	...
	if (source)
		query = [NSString stringWithFormat:@"SELECT RowID FROM packages WHERE source = %u AND identifier = ? ORDER BY date DESC", (unsigned)source.entryID];
	else
...
- (NSString*)installedVersionForPackage:(NSString*)identifier
{
    ATResultSet * res = [[ATDatabase sharedDatabase] executeQuery:@"SELECT version FROM packages WHERE identifier = ? AND isInstalled = 1", identifier];
    NSString* version = nil;
    
    if (res && [res next])
    {
        version = [res stringForColumn:@"version"];
    }
...
- (NSString*)sortPackagesTableName
{
    if (sortPackagesTableName == nil)
        sortPackagesTableName = @"_package_sort";
...
- (void)setSortPackagesTableName:(NSString*)tableName
{
    if (![tableName isEqualToString:sortPackagesTableName])
    {
        ...
        sortPackagesTableName = tableName;
    }
}
...
- (NSString*)packageSectionNamesTableName
{
    if (packageSectionNamesTableName == nil)
        packageSectionNamesTableName = @"_package_section_names";
...
- (void)setPackageSectionNamesTableName:(NSString*)tableName
{
    if (![tableName isEqualToString:packageSectionNamesTableName])
    {
        ...
        packageSectionNamesTableName = tableName;
    }
}
