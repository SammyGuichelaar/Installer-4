...
- (IBAction)emailButtonPressed:(id)sender {
	NSURL* emailURL = [NSURL URLWithString:[NSString stringWithFormat:@"mailto:%@?subject=%@", source.contact, [@"Regarding Install source"  stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]]]];
...
- (IBAction)editingDidEnd:(UITextField*)sender
{
	...
	if (!self.source)
	{
		self.source = [[ATSource alloc] init];
	}
	...
	if ([[newLoc absoluteString] isEqualToString:[self.source.location absoluteString]] || [newURL isEqualToString:@"http://"])
	{
		return;
	}
	...
	[[ATPackageManager sharedPackageManager] refreshSource:self.source];
}
...
- (void)updateSource
{
    ...
    CGRect textRect = [definitionLabel.text boundingRectWithSize:CGSizeMake(rTemp.size.width, 150) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:definitionLabel.font} context:nil];
    
    CGSize sz = textRect.size;
