...
- (NSMethodSignature *)methodSignatureForSelector:(SEL)aSelector
{
	...
	if (!strncmp(sel_getName(aSelector), "_set_", 5))
	{
		ms = [NSMethodSignature signatureWithObjCTypes:"v@:@@"];
	}
	else if (!strncmp(sel_getName(aSelector), "_get_", 5))
	{
...
- (void)forwardInvocation:(NSInvocation *)i
{
    ...
    const char* sel = (const char*)sel_getName([i selector]);
	...
	if (!strncmp(sel, "_set_", 5))
	{
		...
		property = [NSString stringWithUTF8String:propBuffer];
...
- (id)_get:(NSString*)property withClass:(Class)class
{
	...
	NSString* query = [NSString stringWithFormat:@"SELECT %@ FROM %@ WHERE RowID = %llu", property, self.tableName, self.entryID];
	...
	if (set && [set next])
	{
		...
		else if (class == [NSMutableArray class])
		{
            ...
            {
                NSError* err = nil;
                retVal = [NSPropertyListSerialization propertyListWithData:serializedArr options:NSPropertyListMutableContainers format:NULL error:&err];
            }
...
- (BOOL)commit
{
	...
	else					// We do an UPDATE statement
	{
		...
		NSString* query = [NSString stringWithFormat:@"UPDATE %@ SET %@ WHERE RowID = %llu", self.tableName, setStmt, self.entryID];
...
- (void)remove
{
	...
	[[ATDatabase sharedDatabase] executeUpdate:[NSString stringWithFormat:@"DELETE FROM %@ WHERE RowID = %llu", self.tableName, self.entryID]];
