...
+ (ATSourceRefresh*)sourceRefreshWithSourceLocation:(NSString*)location
{
	...
	if (src)
		return [(ATSourceRefresh*)[ATSourceRefresh alloc] initWithSource:src];
...
- (void)taskStart
{
	...
	self.Description = [NSString stringWithFormat:NSLocalizedString(@"Refreshing %@...", @""), self.source.location.absoluteString];
	
	Log(@"ATSourceRefresh: initiate download %@", [sourceURL absoluteString]);
	
	self.download = [[ATURLDownload alloc] initWithRequest:[NSURLRequest requestWithURL:sourceURL] delegate:self];
	
	Log(@"after init download");
}
...
//	[[ATDatabase sharedDatabase] executeUpdate:[NSString stringWithFormat:@"UPDATE packages SET source = NULL WHERE source = %u AND isInstalled = 1", self.source.entryID]];
    NSString* tempTableName = [NSString stringWithFormat:@"TEMP_srcRefresh_0x%X", (unsigned int)self];
...
//	Log(@"Using temporary table %@", tempTableName);
    ...
    [[ATDatabase sharedDatabase] executeUpdate:[NSString stringWithFormat:@"DELETE FROM packages WHERE identifier NOT IN ( SELECT identifier FROM %@ ) AND source = %u AND isInstalled <> 1", tempTableName, (unsigned)self.source.entryID]];
...
- (ATCydiaRepositoryNode*)cydiaRepositoryRootNodeForURL:(NSURL*)sourceURL nodeHTMLFilePath:(NSString*)nodeHTMLPath
{
    if (rootNode == nil)
    {
        rootNode = [[ATCydiaRepositoryNode alloc] initWithURL:sourceURL nodeHTMLFilePath:nodeHTMLPath];
...
- (void)scanDidFinishInNode:(ATCydiaRepositoryNode*)node
{
    ...
    if (packages != nil)
    {
        ...
        NSString* tempTableName = [NSString stringWithFormat:@"TEMP_srcRefresh_0x%@", self];
        ...
        DEBUG_TIMING_RESTART();
        for (NSDictionary __strong* package in packages)
        {
        ...
        // Now remove all packages that did not get to the sources list
        [[ATDatabase sharedDatabase] executeUpdate:[NSString stringWithFormat:@"DELETE FROM packages WHERE identifier NOT IN ( SELECT identifier FROM %@ ) AND source = %llu AND isInstalled <> 1", tempTableName, self.source.entryID]];
        [[ATDatabase sharedDatabase] executeUpdate:[NSString stringWithFormat:@"DROP TABLE %@", tempTableName]];
