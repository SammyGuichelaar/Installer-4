...
NSString* ATSearchResultsUpdatedNotification = @"com.apptapp.install.search.update";
...
- (ATPackage *)packageAtIndex:(unsigned int)index
{
	...
	if(res && [res next]) {
		...
		if (packageID)
			pack = [[ATPackage alloc] initWithID:packageID];
		else
		{
			pack = [[ATPackage alloc] init];
...
- (void)_externalSearch
{
	...
	ATSearchTask* st = [[ATSearchTask alloc] initWithSearch:self];
