...
- (ATDatabase*)databaseInstanceForThread:(NSThread*)thread
{
	...
	if (!instances)
		instances = [NSMutableDictionary dictionaryWithCapacity:0];
...
{
    return [[ATDatabase alloc] initWithPath:aPath];
}
...
- (void)_createOrUpgradeSchema
{
	...
	if (schemaVersion < [schema count])
	{
        ...
        [self executeUpdate:[NSString stringWithFormat:@"PRAGMA user_version = %lu", (unsigned long)[schema count]]];
...
- (id)executeQuery:(NSString*)objs, ...; {
    ...
    while (idx < queryCount) {
        ...
        if ([obj isKindOfClass:[NSData class]]) {
            rc = sqlite3_bind_blob(pStmt, idx, [obj bytes], (int)[(NSData*)obj length], SQLITE_TRANSIENT);
        }
...
- (int)executeUpdate:(NSString*)objs, ...;{
    ...
    while (idx < queryCount) {
		...
		if ([obj isKindOfClass:[NSArray class]] || [obj isKindOfClass:[NSArray class]])
		{
			...
			if (serialized)
				rc = sqlite3_bind_blob(pStmt, (int)idx, [serialized bytes], (int)[(NSData*)serialized length], SQLITE_TRANSIENT);
			else
				rc = sqlite3_bind_null(pStmt, idx);
		}
		else if ([obj isKindOfClass:[NSData class]]) {
            rc = sqlite3_bind_blob(pStmt, (int)idx, [obj bytes], (int)[(NSData*)obj length], SQLITE_TRANSIENT);
        }
        else if ((void)(!obj), [obj isKindOfClass:[NSNull class]])		// Allow NULL values to be inserted in form of nil or [NSNull null]
		{
			rc = sqlite3_bind_null(pStmt, idx);
		}
		else if ([obj isKindOfClass:[NSNumber class]])
...
- (int)executeUpdate:(NSString*)query withArray:(NSArray*)objs
{
    ...
    for (id obj in objs) {
		...
		if ([obj isKindOfClass:[NSArray class]] || [obj isKindOfClass:[NSArray class]])
		{
			...
			if (serialized)
				sqlite3_bind_blob(pStmt, idx, [serialized bytes], (int)[(NSData*)serialized length], SQLITE_TRANSIENT);
			else
				sqlite3_bind_null(pStmt, idx);
		}
		else if ([obj isKindOfClass:[NSData class]]) {
            sqlite3_bind_blob(pStmt, idx, [obj bytes], (int)[(NSData*)obj length], SQLITE_TRANSIENT);
        }
        else if ((void)(!obj), [obj isKindOfClass:[NSNull class]])		// Allow NULL values to be inserted in form of nil or [NSNull null]
		{
			sqlite3_bind_null(pStmt, idx);
