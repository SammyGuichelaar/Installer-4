...
- (void)downloadDidFinish:(ATURLDownload *)dl
{
	...
	if (!dict)
	{
		...
		NSError* err = [NSError errorWithDomain:AppTappErrorDomain code:kATErrorPackageInfoDecodeFailed userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Unable to decode package more info at %@", self.package.moreURL], NSLocalizedDescriptionKey, self.package.identifier, @"packageID", nil]];
...
//		self.package.customInfoURL = nil;	
    ...
    if ([dict objectForKey:@"screenshots"] && [[dict objectForKey:@"screenshots"] isKindOfClass:[NSArray class]])
        self.package.screenshots = [NSMutableArray arrayWithArray:[dict objectForKey:@"screenshots"]];
