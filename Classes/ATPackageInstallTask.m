...
- (void)taskStart
{
	...
	self.download = [[ATURLDownload alloc] initWithRequest:[NSURLRequest requestWithURL:sourceURL] delegate:self resumeable:YES];
}
...
- (void)download:(ATURLDownload *)dl didCreateDestination:(NSString *)path
{
	...
	self.downloadBytes = [[[NSFileManager defaultManager] attributesOfItemAtPath:path error:nil] fileSize];
}
...
- (void)downloadDidFinish:(ATURLDownload *)dl
{
	...
	unsigned long long fileSize = [[[NSFileManager defaultManager] attributesOfItemAtPath:self.tempFileName error:nil] fileSize];
    ...
    if (!self.script)
    {
        self.script = [[ATScript alloc] initWithDelegate:self];
    ...
    [[ATDatabase sharedDatabase] executeUpdate:[NSString stringWithFormat:@"DELETE FROM packages WHERE identifier = ? AND RowID <> %u AND isInstalled = 1", (unsigned)self.package.entryID], self.package.identifier, nil];
